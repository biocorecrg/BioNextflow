...
makeDDSFromMatrix <- function(input, desc, field) {
	...
	# Convert star Reads Per Gene in count matrix
...
makeHeatmapDE <- function(results, dds, assay_field_raw, treat, ctrl, pcut, l2fc, h, w) {
...
plotPCA_plus <- function (object, intgroup = "condition", ntop = 500, returnData = FALSE, pcnum) {
...
PCA_data <- function (object, intgroup = "condition", ntop = 500, pcnum)
{
    rv <- rowVars(assay(object), useNames = TRUE)
    select <- order(rv, decreasing = TRUE)[seq_len(min(ntop,length(rv)))]
    pca <- prcomp(t(assay(object)[select, ]))
    ...
    data <- data.frame(group = group, intgroup.df, name = colnames(object))
	for (i in 1:pcnum) {
	data[[paste0("PC", i)]] <- pca$x[, i]
	}
    attr(data, "percentVar") <- percentVar[1:4]
    pv <- round(100*attr(data, "percentVar"))
	PC_names <- c(paste("PC", 1:4, sep=""))
	pv_df<-data.frame(PC_names, pv)

return(list(data,pv_df))
}


printCounts <- function(vsd, desc) {

	resMerged<-merge(as.data.frame(assay(vsd)), desc, all.x=F, by.x=0, by.y="gene.id", sort=FALSE)
	resMerged$ID<-NULL
	write.csv(resMerged, file="vst.genes", row.names = FALSE)          
    ...
    resMerged2$ID<-NULL
    write.csv(resMerged2, file="norm_counts.genes", row.names = FALSE)

	resMerged3<-merge(as.data.frame(rawcounts) ,desc, all.x=F, by.x=0, by.y="gene.id", sort=FALSE)
    resMerged3$ID<-NULL
 
    write.csv(resMerged3, file="raw_counts.genes", row.names = FALSE)
